...
import {getCurrentBranchName, assembleGitPush, gitPush} from './git-adapter';
...
async function main(program) {
	...
	const force = program['force'] ? program['force'] : false;
	const dryRun = program['dryRun'] ? program['dryRun'] : false;
	const isProduction = program['production'] ? program['production'] : false;
	...
	const currGitBranch = await getCurrentBranchName();
	...
	const gitPushParameters = assembleGitPush(gitTarget, currGitBranch, gitTargetBranch, force, extraOptions);
	console.log(`The exact command is: 	git ${gitPushParameters.join(' ')}`);

	if (isProduction) {
		const value = await prompt(`CAUTION: You are updating a production branch! Proceed? (yes | NO) `);

		if (value !== 'yes' || value !== 'y') {
			console.log('Git Push canceled by user');
			process.exit(0);
		}
	}
